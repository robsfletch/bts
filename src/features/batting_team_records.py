...
def main(interim):
    ...
    events['BAT_TEAM_ID'] = events['HOME_TEAM_ID']
    events.loc[events.BAT_HOME_ID == 0, 'BAT_TEAM_ID'] = events['AWAY_TEAM_ID']

    record = events.groupby(['BAT_TEAM_ID', 'year']).agg({
        'GAME_ID': 'nunique',
        'AB_FL': 'sum',
        'H': 'sum',
        '2B': 'sum',
    ...
    record.to_pickle(Path(interim) / 'batting_team_records.pkl')
